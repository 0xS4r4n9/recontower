#!/usr/bin/env python3
"""
ReconTower - Network Auditing Tool
Author: 0xS4r4n9
Version: 1.0.0
"""
...
# Silence paramiko's noisy internal transport errors (banner resets, etc.)
# Fully silence paramiko — Kali ships an older system paramiko that prints
# banner errors directly from a background thread (not via the logger),
# so we must suppress at BOTH the logging level AND stderr.
import io, threading

class _ParamikoStderrFilter(io.TextIOWrapper):
    """Drop any stderr lines that originate from paramiko internals."""
    _SKIP = (
    ...
    def write(self, s):
        if any(k in s for k in self._SKIP):
...
sys.stderr = _ParamikoStderrFilter(sys.stderr)

for _log_name in (paramiko, paramiko.transport, paramiko.transport.sftp,
                  paramiko.packet, paramiko.auth_handler):
        ...
        for user in self.COMMON_USERS[:5]:
            for pw in self.COMMON_PASSES[:8]:
                ...
                try:
                    ...
                    # Wrong password — expected, keep trying
                    pass
                except paramiko.ssh_exception.SSHException:
                    # Banner read error, connection reset, rate limiting — stop
                    break
                except (ConnectionResetError, ConnectionRefusedError, OSError):
                    # Network-level reset — stop
                    break
                except Exception:
                    break
                finally:
...
class ReportGenerator:
    def __init__(self, hosts_data, findings, output_path="recontower_report.pdf"):
    ...
    def generate_pdf(self):
        ...
        story.append(Paragraph("ReconTower", title_style))
        ...
        meta = [
            ...
            ["Generated By", "ReconTower v1.0 by 0xS4r4n9"],
        ...
        story.append(Paragraph(
            f"ReconTower performed an automated security audit of {len(self.hosts_data)} host(s). "
        ...
        story.append(Paragraph(
            f"<i>Report generated by ReconTower v1.0 by 0xS4r4n9 on "
...
class ReconTower:
    ...
    def run(self):
        ...
        if not self.args.no_report:
            report_path = self.args.output or f"recontower_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        ...
        console.print()
        console.print(Panel.fit(
            f"[bold green]ReconTower scan complete![/bold green]\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="ReconTower — Network Auditing Tool by 0xS4r4n9",
...
Examples:
  ...
  sudo python3 recontower.py -t 10.0.0.1 --fast --no-exploits
  sudo python3 recontower.py -t 192.168.1.100 -o client_report.pdf
        """
    )
    ...
    parser.add_argument("--version", action="version", version="ReconTower v1.0 by 0xS4r4n9")
